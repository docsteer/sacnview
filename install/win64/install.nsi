; Installation Script for sACNView 64bit
...
;You may use these definitions also, when you want to set up the InstallDirRegKey,
...
!finalize '${__FILEDIR__}\sign_installer.bat "%1" ' = 0
!uninstfinalize '${__FILEDIR__}\sign_installer.bat "%1" ' = 0
...
;close the opened previously uninstall log macros block.
	...
	WriteRegStr ${INSTDIR_REG_ROOT} "${INSTDIR_REG_KEY}" "DisplayIcon" "$INSTDIR\sACNView.exe"
...
Function .onInit
		...
		${IfNot} ${AtLeastWin7}
		${OrIfNot} ${RunningX64}
			MessageBox MB_OK "64bit Windows 7 or above is required to run ${PRODUCT_NAME}"
			Quit
		${EndIf}
