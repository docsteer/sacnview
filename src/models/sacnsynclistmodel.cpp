    ...
    : QAbstractItemModel(parent)
{
    ...
    for (auto weakListener : sACNManager::Instance().getListenerList())
    {
        ...
        for (auto source : listener->getSourceList())
            if (!source->active.Expired() && source->synchronization) addSyncAddress(source->synchronization);
    }

    // Future Sync Addresses
    connect(
        sACNSynchronizationRX::getInstance(),
        &sACNSynchronizationRX::newSyncAddress,
        this,
        &sACNSyncListModel::addSyncAddress);
    // -- Source with no known Synchroniser
    connect(
        &sACNManager::Instance(),
        &sACNManager::sourceFound,
        this,
        [=](quint16 universe, sACNSource * source)
        {
            Q_UNUSED(universe);
    ...
    connect(
        sACNSynchronizationRX::getInstance(),
        &sACNSynchronizationRX::expiredSyncAddress,
        this,
        [=](tsyncAddress syncAddress)
        {
            if (isSyncAddressEmpty(syncAddress)) removeSyncAddress(syncAddress);
        },
        Qt::DirectConnection);
    // -- Source
    connect(
        &sACNManager::Instance(),
        &sACNManager::sourceLost,
        this,
        [=](quint16 universe, sACNSource * source)
        {
            Q_UNUSED(universe);
            if (isSyncAddressEmpty(source->synchronization)) removeSyncAddress(source->synchronization);
        });
}
...
{
    item * parentItem;
    if (parent.column() > columnCount()) return 0;
...
void sACNSyncListModel::addRow(item * newItem)
{
    ...
    item * parentItem = newItem->parent() ? newItem->parent() : rootItem;

    QModelIndex parent = parentItem != rootItem ? createIndex(parentItem->row(), parentItem->column(), parentItem)
                                                : QModelIndex();
...
void sACNSyncListModel::removeRow(item * oldItem)
{
    ...
    item * parentItem = oldItem->parent() ? oldItem->parent() : rootItem;
    ...
    // Help message management
    if (parentItem->type() != itemType_HelpMsg && !parentItem->childCount()) addHelpMsg(parentItem);
}
...
{
    if (m_parentItem) return static_cast<int>(m_parentItem->m_children.indexOf(const_cast<item *>(this)));
...
{
    if (index > static_cast<size_t>(m_children.size())) return Q_NULLPTR;
...
void sACNSyncListModel::addHelpMsg(item * parent)
{
    ...
    {
        case itemType_Root:
            messages.insert(Qt::DisplayRole, QVariant(tr("No known details")));
            messages.insert(
                Qt::ToolTipRole,
                QVariant(
                    tr("Synchronisation relies upon\n"
                       "E1-31:2016 universe synchronisation packets\n"
                       "not all sources support or require this")));
            break;

        case itemType_SyncAddress_Source:
            messages.insert(Qt::DisplayRole, QVariant(tr("No known sources")));
            messages.insert(
                Qt::ToolTipRole,
                QVariant(
                    tr("Synchronisation provides no discovery method\n"
                       "A universe listener must already be present")));
            break;

        case itemType_SyncAddress_Synced:
            messages.insert(Qt::DisplayRole, QVariant(tr("No known universes")));
            messages.insert(
                Qt::ToolTipRole,
                QVariant(
                    tr("Synchronisation provides no discovery method\n"
...
void sACNSyncListModel::RemoveHelpMsg(item * parent)
{
    ...
    for (auto child : parent->children())
        if (child->type() == itemType_HelpMsg) removeRow(child);
}
...
bool sACNSyncListModel::isSyncAddressEmpty(tsyncAddress syncAddress)
{
    ...
    for (auto weakListener : sACNManager::Instance().getListenerList())
    {
        auto listener = weakListener.toStrongRef();
        for (auto source : listener->getSourceList())
            if (!source->active.Expired() && source->synchronization == syncAddress) return false;
    }
...
void sACNSyncListModel::addSyncAddress(tsyncAddress syncAddress)
{
    ...
    for (auto weakListener : sACNManager::Instance().getListenerList())
    {
        auto listener = weakListener.toStrongRef();
        for (auto source : listener->getSourceList())
            if (!source->active.Expired() && source->synchronization == syncAddress)
            {
    ...
    connect(
        sACNSynchronizationRX::getInstance(),
        &sACNSynchronizationRX::newSource,
        this,
        &sACNSyncListModel::addSyncSourceCID,
        Qt::DirectConnection);

    // Expired Sync sources
    connect(
        sACNSynchronizationRX::getInstance(),
        &sACNSynchronizationRX::expiredSource,
        this,
        &sACNSyncListModel::removeSyncSourceCID,
        Qt::DirectConnection);
}
...
QVariant sACNSyncListModel::item_SyncAddress::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(tr("Sync Address %1").arg(syncAddress()));
...
QVariant sACNSyncListModel::item_SyncAddress_Source::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(tr("Synchroniser"));
...
sACNSyncListModel::itemPair_SyncAddress_Source_CID sACNSyncListModel::getSyncSourceCID(
    tsyncAddress syncAddress,
    CID cid)
{
    ...
    if (!itemSyncAddress) return ret;

    for (const auto child : itemSyncAddress->children())
        if (child->type() == itemType_SyncAddress_Source) ret.parentItem = child;
    if (!ret.parentItem) return ret;
...
QVariant sACNSyncListModel::item_SyncAddress_Source_CID::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(QString("CID: %1").arg(m_cid));
...
QVariant sACNSyncListModel::item_SyncAddress_Source_IP::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(QString("IP: %1").arg(m_address.toString()));
...
sACNSyncListModel::item_SyncAddress_Source_FPS::item_SyncAddress_Source_FPS(FpsCounter * fps, item * parent)
    : item(parent), m_fps(fps)
{
    connect(
        m_fps,
        &FpsCounter::updatedFPS,
        this,
        [=]()
        {
...
QVariant sACNSyncListModel::item_SyncAddress_Source_FPS::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(QString("FPS: %1").arg(m_fps->FPS()));
...
QVariant sACNSyncListModel::item_SyncAddress_Synced::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(tr("Synced Universes"));
...
sACNSyncListModel::itemPair_SyncAddress_Synced_Universe sACNSyncListModel::getSyncedUniverse(
    tsyncAddress syncAddress,
    quint16 universe)
{
    ...
    for (const auto child : itemSyncAddress->children())
        if (child->type() == itemType_SyncAddress_Synced) ret.parentItem = child;
    if (!ret.parentItem) return ret;
...
void sACNSyncListModel::addSyncedUniverse(tsyncAddress syncAddress, quint16 universe)
{
    ...
    // Create new
    if (itemPair.parentItem) addRow(new item_SyncAddress_Synced_Universe(universe, itemPair.parentItem));
}
...
QVariant sACNSyncListModel::item_SyncAddress_Synced_Universe::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        case Qt::DisplayRole: return QVariant(tr("Universe %1").arg(universe()));

        case static_cast<Qt::ItemDataRole>(Roles::Universe): return universe();
...
sACNSyncListModel::itemPair_SyncAddress_Synced_Universe_Source sACNSyncListModel::getSyncedUniverseSource(
...
void sACNSyncListModel::addSyncedUniverseSource(const sACNSource * source)
{
    ...
    // Create new
    if (itemPair.parentItem) addRow(new item_SyncAddress_Synced_Universe_Source(source, itemPair.parentItem));
}
...
QVariant sACNSyncListModel::item_SyncAddress_Synced_Universe_Source::data(Qt::ItemDataRole role) const
{
    switch (role)
    {
        ...
        case static_cast<Qt::ItemDataRole>(Roles::Universe): return m_source->universe;
...
bool sACNSyncListModel::proxy::lessThan(const QModelIndex & left, const QModelIndex & right) const
{
    ...
    if (leftModel->type() != rightModel->type()) return leftModel->type() < rightModel->type(); // Sort by type enum
    ...
    switch (leftModel->type())
    {
        ...
        case itemType_SyncAddress_Source:
        case itemType_SyncAddress_Synced: return false;
    }
