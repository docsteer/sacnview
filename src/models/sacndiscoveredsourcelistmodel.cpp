...
void sACNDiscoveredSourceListModel::modelContainer::addUniverse(const CID & cid, quint16 universe)
{
    ...
    if (!universes.at(sourceRow).contains(universe)) universes[sourceRow].append(universe);
}
...
CID sACNDiscoveredSourceListModel::modelContainer::getCID(unsigned int row) const
{
    ...
    if (row >= static_cast<unsigned int>(sources.size())) return CID();

    return sources.at(row);
}

sACNDiscoveredSourceListModel::modelContainer::universe_t sACNDiscoveredSourceListModel::modelContainer::getUniverse(
    const CID & cid,
    unsigned int row) const
{
    ...
    const auto sourceRow = sources.indexOf(cid);
    if (sourceRow >= universes.size()) return 0;

    if (row >= static_cast<unsigned int>(universes.at(sourceRow).size())) return 0;
...
sACNDiscoveredSourceListModel::sACNDiscoveredSourceListModel(QObject * parent)
    ...
    , m_displayDDOnlySource(Preferences::Instance().GetETCDisplayDDOnly())
{
    ...
    while (it != m_discoveryInstance->getDiscoveryList().constEnd()) newSource(it.key());
    ...
    connect(
        m_discoveryInstance,
        &sACNDiscoveryRX::expiredUniverse,
        this,
        &sACNDiscoveredSourceListModel::expiredUniverse);
}
...
void sACNDiscoveredSourceListModel::newSource(CID cid)
{
    if (m_sources.getRow(cid) == -1)
    {
        ...
        if (appendSource) beginInsertRows(QModelIndex(), childRow, childRow);
    ...
    switch (role)
    {
        ...
        case Qt::DisplayRole:
            if (index.parent().isValid())
            {
                ...
                if (universe < MIN_SACN_UNIVERSE) return tr("Error");
...
sACNDiscoveredSourceListProxy::sACNDiscoveredSourceListProxy(QObject * parent)
    : QSortFilterProxyModel(parent), collator()
{
