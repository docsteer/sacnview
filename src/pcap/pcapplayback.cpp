...
PcapPlayback::PcapPlayback(QWidget * parent)
    : QWidget(parent), ui(new Ui::PcapPlayback), sender(NULL)
{
    ui->setupUi(this);
}
...
void PcapPlayback::on_btnOpen_clicked()
{
    ...
    {
        QTime elasped(0, 0, 0);
        ui->lblFileTime->setText(
            QString("%1").arg(elasped.addMSecs(pkt_start_time.msecsTo(pkt_end_time)).toString("hh:mm:ss.zzz")));
    }
    updateUIBtns();
...
{
    QMessageBox::warning(this, tr("PCap Playback"), errorMessage, QMessageBox::Ok);
