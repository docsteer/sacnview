...
{
    for (QByteArray & preset : m_presets) preset = DefaultByteArrayDmx;
    for (size_t i = 0; i < m_priorityPresets.size(); ++i) m_priorityPresets[i] = DefaultByteArrayPriority(i);
    ...
    if (pref_index == -1) pref_index = args.lastIndexOf(QStringLiteral("/ini"));

    if (pref_index != -1 && pref_index < args.size() - 1) m_settings_file = args[pref_index + 1];
...
QNetworkInterface Preferences::networkInterface() const
{
    if (!m_interface.isValid())
    {
        ...
        {
            if (interface.flags().testFlag(QNetworkInterface::IsLoopBack)) return interface;
        }
    }
    return m_interface;
}
...
bool Preferences::interfaceSuitable(const QNetworkInterface & iface)
{
                ...
                && iface.flags().testFlag(QNetworkInterface::IsLoopBack))))
    {
        ...
        {
            if (addr.ip().protocol() == QAbstractSocket::IPv4Protocol) return true;
        }
    }
    return false;
}
...
QColor Preferences::colorForCID(const CID & cid) const
{
    ...
    if (existing != m_cidToColor.end()) return existing.value();
...
{
    // Negative means "none"
    if (nLevelInDecimal < 0) return QStringLiteral("---");
...
{
    if (m_settings_file.isEmpty()) return QSettings();
...
void Preferences::loadPreferences()
{
    ...
    if (settings.contains(S_INTERFACE_ADDRESS))
    {
        ...
        else
        {
            ...
            for (const auto & i : ifaceList)
                if (i.hardwareAddress() == mac) m_interface = i;
        }
    }

    m_interfaceListenAll = settings.value(S_LISTEN_ALL, m_interfaceListenAll).toBool();
    m_nDisplayFormat = static_cast<DisplayFormat>(
        settings.value(S_DISPLAY_FORMAT, static_cast<int>(m_nDisplayFormat)).toInt());
    ...
    m_pathwaySecureTxSequenceType = settings.value(S_PATHWAYSECURE_TX_SEQUENCE_TYPE, m_pathwaySecureTxSequenceType)
                                        .toUInt();
    m_pathwaySecureTxSequenceBootCount =
        settings.value(S_PATHWAYSECURE_TX_SEQUENCE_BOOT_COUNT, m_pathwaySecureTxSequenceBootCount).toUInt();
