...
sACNEffectEngine::sACNEffectEngine(sACNManager::tSender sender)
    : QObject(NULL)
...
void sACNEffectEngine::setMode(sACNEffectEngine::FxMode mode)
{
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setMode", Q_ARG(sACNEffectEngine::FxMode, mode));
    else
        m_mode = mode;
...
void sACNEffectEngine::setStartAddress(quint16 start)
{
    ...
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setStartAddress", Q_ARG(quint16, start));
    else
    {
...
void sACNEffectEngine::setEndAddress(quint16 end)
{
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setEndAddress", Q_ARG(quint16, end));
    else
    {
        // Limit to sender slot count
...
void sACNEffectEngine::setRange(quint16 start, quint16 end)
{
    ...
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setRange", Q_ARG(quint16, start), Q_ARG(quint16, end));
    else
    {
...
void sACNEffectEngine::setText(QString text)
{
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setText", Q_ARG(QString, text));
    else
    {
        m_text = text;
...
void sACNEffectEngine::setDateStyle(sACNEffectEngine::DateStyle style)
{
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setDateStyle", Q_ARG(sACNEffectEngine::DateStyle, style));
    else
        m_dateStyle = style;
}
...
void sACNEffectEngine::setRate(qreal hz)
{
    // Make this method thread-safe
    if (QThread::currentThread() != this->thread())
        QMetaObject::invokeMethod(this, "setRate", Q_ARG(qreal, hz));
    else
    {
        m_rate = hz;
        int msTime = 1000 / hz;
