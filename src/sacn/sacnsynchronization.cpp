...
sACNSynchronizationTX::sACNSynchronizationTX(CID cid, tsyncAddress syncAddress)
    : QObject(), m_cid(cid), m_syncAddress(syncAddress)
{
    // Create base packet
...
void sACNSynchronizationTX::createSynchronizationPacket()
{
    ...
    VHD_PackLength(
        (quint8 *)m_pbuf.data() + ROOT_FLAGS_AND_LENGTH_ADDR,
        bufSize - ROOT_FLAGS_AND_LENGTH_ADDR,
        false); // Length
    ...
    VHD_PackLength(
        (quint8 *)m_pbuf.data() + SYNC_FLAGS_AND_LENGTH_ADDR,
        bufSize - SYNC_FLAGS_AND_LENGTH_ADDR,
        false); // Length
...
{
    if (!m_instance) m_instance = new sACNSynchronizationRX();
}
...
sACNSynchronizationRX::sACNSynchronizationRX()
    : QObject(), m_thread(new QThread(this)), m_expiredTimer(new QTimer(this))
{
    ...
    m_expiredTimer->setInterval(
        E131_NETWORK_DATA_LOSS_TIMEOUT + (E131_NETWORK_DATA_LOSS_TIMEOUT / 4)); // Expire after 125% of time
...
void sACNSynchronizationRX::processPacket(
    const quint8 * pbuf,
    size_t buflen,
    ...
    const QHostAddress & sender)
{
    ...
    m_synchronizationSources[syncAddress][cid].dataLoss.SetInterval(
