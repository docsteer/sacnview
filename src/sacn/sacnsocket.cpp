...
sACNRxSocket::sACNRxSocket(QNetworkInterface iface, QObject * parent)
    : QUdpSocket(parent), m_interface(iface)
{}
...
# ifdef Q_OS_WIN
#  pragma message( \
      "setMulticastInterface() fails to bind to correct interface on systems running IPV4 and IPv6 with QT <= 5.8.0")
# else
...
# endif
    ...
    if (ok)
    {
        qDebug()
            ...
            << ": Bound to interface:"
            << multicastInterface().name();
        qDebug()
            << "sACNRxSocket"
            << m_universe
            << QThread::currentThreadId()
            << ": Joined Multicast Group:"
            << QHostAddress(multicastAddr.GetV4Address()).toString();
    }
    else
    {
        close();
        ...
        if (ifaceAddr.ip().protocol() == QAbstractSocket::IPv4Protocol)
        {
            ...
            if (ok)
            {
                ...
                setSocketOption(
                    ...
                    QVariant(Preferences::Instance().GetMulticastTtl()));
    ...
    if (writeDatagramLoopbackTest(host, localAddress()))
    {
        ...
        for (const auto & weakListener : sACNManager::Instance().getListenerList())
        {
            ...
            if (listener)
            {
                ...
                {
                    listener->processDatagram(QByteArray(data, len), QHostAddress(host), localAddress());
                }
            }
        }
    }
