...
public:
    ...
    void setSendFrequency(
        uint handle,
        float minimum = E1_11::MIN_REFRESH_RATE_HZ,
        float maximum = E1_11::MAX_REFRESH_RATE_HZ);
...
private:
    ...
    struct universe
    {
        ...
        bool isdirty;
        quint8
            num_suppress_remaining; //Packets left before we enter Transmission rate supression (E1.31:2016 6.6.2 (clause 1) logic)
        ...
        universe()
            ...
            , psend(NULL)
            , isdirty(false)
            , num_suppress_remaining(0)
            , version(sACNProtocolRelease)
            , cid()
            , password("")
        {}
    };
