...
{
    if (!m_instance) m_instance = new sACNDiscoveryTX();
}
...
sACNDiscoveryTX::sACNDiscoveryTX()
    : QObject(), m_mutex(new QMutex()), m_sendTimer(new QTimer(this))
{
    // Socket
...
void sACNDiscoveryTX::sendDiscoveryPacket()
{
    ...
    for (const auto & cid : cidList)
    {
        ...
        {
            i.next();
            if (!sACNManager::Instance().getSender(i.value(), cid)->isSending()) i.remove();
        }
        if (universeList.isEmpty()) continue; // No active universes being sent by this CID
        ...
        QString senderName;
        if (universeList.count() > 0) senderName = sACNManager::Instance().getSender(universeList.front(), cid)->name();
        ...
        for (uint8_t page = 0; page < page_count; page++)
        {
            ...
            VHD_PackLength(
                (quint8 *)pbuf.data() + ROOT_FLAGS_AND_LENGTH_ADDR,
                pbufSize - ROOT_FLAGS_AND_LENGTH_ADDR,
                false); // Length
            ...
            VHD_PackLength(
                (quint8 *)pbuf.data() + FRAMING_FLAGS_AND_LENGTH_ADDR,
                pbufSize - FRAMING_FLAGS_AND_LENGTH_ADDR,
                false); // Length
            PackBUint32((quint8 *)pbuf.data() + FRAMING_VECTOR_ADDR, VECTOR_E131_EXTENDED_DISCOVERY); // Vector
            strncpy(
                pbuf.data() + SOURCE_NAME_ADDR,
                senderName.toLatin1().constData(),
                SOURCE_NAME_SIZE - 1); // Source Name

            // Universe discovery layer
            VHD_PackFlags((quint8 *)pbuf.data() + DISCO_FLAGS_AND_LENGTH_ADDR, false, false, false); // Flags
            VHD_PackLength(
                (quint8 *)pbuf.data() + DISCO_FLAGS_AND_LENGTH_ADDR,
                pbufSize - DISCO_FLAGS_AND_LENGTH_ADDR,
                false); // Length
...
{
    if (!m_instance) m_instance = new sACNDiscoveryRX();
}
    ...
    , m_expiredTimer(new QTimer(this))
{
    ...
    m_expiredTimer->setInterval(
        E131_UNIVERSE_DISCOVERY_INTERVAL + (E131_UNIVERSE_DISCOVERY_INTERVAL / 4)); // Expire after 125% of time
...
void sACNDiscoveryRX::processPacket(const quint8 * pbuf, size_t buflen)
{
    ...
    // Check length
    if (!(buflen >= E131_UNIVERSE_DISCOVERY_SIZE_MIN) || !(buflen <= E131_UNIVERSE_DISCOVERY_SIZE_MAX)) return;
    ...
    if (buflen != E131_UNIVERSE_DISCOVERY_SIZE_MIN)
    {
        for (quint16 n = 0; n < buflen - DISCO_LIST_UNIVERSE_ADDR; n += sizeof(quint16))
        {
            ...
            m_discoveryList[cid]->Universe[universe].SetInterval(
                std::chrono::milliseconds(E131_UNIVERSE_DISCOVERY_INTERVAL));
        }
    }
    else
    {
        qDebug()
            << "DiscoveryRX : Discovery packet received with empty universe list - CID"
            << CID::CIDIntoQString(cid);
    }
}
