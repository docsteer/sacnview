...
void PreferencesDialog::showEvent(QShowEvent * e)
{
    ...
    for (const QNetworkInterface & iface : interfaces)
    {
        ...
        if (Preferences::Instance().interfaceSuitable(iface))
        {
            ...
            ui->cmbNetInterface->addItem(QStringLiteral("%1 (%2)").arg(iface.humanReadableName()).arg(ipString));
...
void PreferencesDialog::on_buttonBox_accepted()
{
    ...
    if (ui->cbETCDisplayDDOnlys->isChecked() != p.GetETCDisplayDDOnly())
    {
    ...
    // Transmit timeout
    int seconds = ui->NumOfHoursOfSacn->value() * 60 * 60
        + ui->NumOfMinOfSacn->value() * 60
        + ui->NumOfSecOfSacn->value();
    if (!ui->gbTransmitTimeout->isChecked()) seconds = 0;
    p.SetNumSecondsOfSacn(seconds);

    // Default source name
    p.SetDefaultTransmitName(ui->leDefaultSourceName->text());

    if (ui->cbTxRateOverride->isChecked() != p.GetTXRateOverride()) requiresRestart = true;
    p.SetTXRateOverride(ui->cbTxRateOverride->isChecked());

    if (ui->cbTxBadPriority->isChecked() != p.GetTXBadPriority()) requiresRestart = true;
    ...
    for (const auto & weakListener : listenerList)
    {
        ...
        if (listener) listener->doFullMerge();
    }
