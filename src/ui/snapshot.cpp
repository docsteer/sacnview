...
Snapshot::PlaybackBtn::PlaybackBtn(QWidget * parent)
    : QToolButton(parent)
{
    setPlay();
}
...
Snapshot::InfoLbl::InfoLbl(QWidget * parent)
    : QLabel(parent)
{
    setText(stSetup);
}
...
void Snapshot::InfoLbl::setText(Snapshot::state state)
{
    switch (state)
    {
        case stSetup:
            ...
            break;
        case stCountDown5: Q_FALLTHROUGH();
        case stCountDown4: Q_FALLTHROUGH();
        case stCountDown3: Q_FALLTHROUGH();
        case stCountDown2: Q_FALLTHROUGH();
        case stCountDown1: setText(tr("Capturing snapshot in...")); break;
        case stReadyPlayback: Q_FALLTHROUGH();
        case stReplay: setText(tr("Press Play to playback snapshot")); break;
        case stPlayback: setText(tr("Replaying Data")); break;
    }
}
...
{
    bool ok;
    QString rawInput = QInputDialog::getText(
        this,
        tr("Capture universes"),
        tr("List of capture universes"),
        QLineEdit::Normal,
        "1,2,3,4-10",
        &ok);
        ...
        if (item.contains("-"))
        {
            ...
            for (unsigned int universe = first; universe <= last; ++universe)
            {
                if (universe >= MIN_SACN_UNIVERSE && universe <= MAX_SACN_UNIVERSE)
                    if (!universeList.contains(universe)) universeList << universe;
            }
        }
        else
        {
    ...
    {
        auto warningReply = QMessageBox::warning(
            this,
            tr("Warning"),
        ...
        if (warningReply == QMessageBox::No) return;
    }
...
void Snapshot::addUniverse(quint16 universe)
{
    ...
    {
        if (snapshot->getUniverse() == universe) return;
    }
...
void Snapshot::btnPlay_update(bool updateState)
{
    ...
    for (const auto snap : std::as_const(m_snapshots))
        if (snap->isPlaying()) displayPause = true;
    if (displayPause)
    {
        ui->btnPlay->setPause();
        if (updateState && m_state != stPlayback) setState(stPlayback);
    }
    else
    {
        ui->btnPlay->setPlay();
        if (updateState && m_state != stReplay) setState(stReplay);
    }
}
...
{
    for (const auto snap : std::as_const(m_snapshots)) snap->playSnapshot();
}

void Snapshot::stopSnapshot()
{
    for (const auto snap : std::as_const(m_snapshots)) snap->stopSnapshot();
}
...
{
    if ((m_state == stReadyPlayback) || (m_state == stReplay) || (m_state == stSetup))
...
void Snapshot::updateMatchIcon()
{
    ...
    {
        bool allmatch = true;
        for (const auto snap : std::as_const(m_snapshots)) allmatch &= snap->isMatching();
        if (allmatch)
        {
