    ...
    int priority)
{
    ...
    connect(
        fxEngine,
        &sACNEffectEngine::destroyed,
        [this](QObject * obj)
        {
            this->m_fxEngines.removeAll(static_cast<sACNEffectEngine *>(obj));
        }); // Remove fxEngine from List when destroyed
    ...
    fxEngine->run();
    sender->setName(name);
    if (startSending) sender->startSending();
    ...
    ui->tableWidget->setColumnWidth(COL_EFFECT, std::max(ui->tableWidget->columnWidth(COL_EFFECT), cb->width()));
...
void MultiUniverse::on_btnAddRow_pressed()
{
    ...
    if (m_senders.count() > 0) nextUniverse = m_senders.last()->universe() + 1;
...
void MultiUniverse::on_btnRemoveRow_pressed()
{
    ...
    if (m_fxEngines.count() > row) m_fxEngines[row]->deleteLater();
}
...
void MultiUniverse::on_tableWidget_cellChanged(int row, int column)
{
    if (column == COL_NAME)
    {
        if (ui->tableWidget->item(row, column)) m_senders[row]->setName(ui->tableWidget->item(row, column)->text());
    }
    if (column == COL_CONTROL)
    {
        if (ui->tableWidget->item(row, column)) m_fxEngines[row]->setText(ui->tableWidget->item(row, column)->text());
    }
}
