...
NewVersionDialog::NewVersionDialog(QWidget * parent)
    : QDialog(parent), ui(new Ui::NewVersionDialog)
{
    ui->setupUi(this);
...
void NewVersionDialog::setNewVersionNumber(const QString & version)
{
    ui->lblVersion->setText(
        tr("sACNView %1 is available (you have %2). Would you like to download it now?").arg(version, VERSION));
...
{
    ui->lblDownloadInfo->setText(tr("Downloading %1 from %2").arg(m_newVersion, m_dlUrl));
...
{
    // Begin the download
    if (m_dlFile.isOpen()) m_dlFile.close();
...
void NewVersionDialog::progress(qint64 bytes, qint64 total)
{
    ...
    auto strProgress = tr("%1 of %2")
                           .arg(this->locale().formattedDataSize(bytes), this->locale().formattedDataSize(total));
#endif
...
void NewVersionDialog::on_btnExitInstall_pressed()
{
    ...
    if (!ok)
        QMessageBox::warning(
            this,
            tr("Couldn't Run Installer"),
            tr("Unable to run installer - please run %1").arg(m_storagePath));
    qApp->exit();
}
...
void VersionCheck::replyFinished(QNetworkReply * reply)
{
    ...
    if (reply->error())
    {
        ...
        switch (reply->error())
        {
            ...
            case QNetworkReply::TimeoutError: break;
            default:
    ...
    else
    {
        ...
        else
        {
            ...
            for (const QJsonValue & jValue : jArray)
            {
                ...
                QDate objectDate =
                    locale.toDateTime(jObj["published_at"].toString(), QString("yyyy-MM-dd'T'hh':'mm':'ss'Z'")).date();
                ...
                if (myDate.isValid() && objectDate.isValid())
                {
                    if (VERSION != remote_version)
                    {
                        ...
                        {
                            if (PRERELEASE || !remote_is_prerelease)
                            { // If prerelease, always upgrade; if not, only upgrade to non-prerelease
                                // Newer!
