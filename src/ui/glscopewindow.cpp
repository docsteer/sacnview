...
GlScopeWindow::GlScopeWindow(int universe, QWidget * parent)
    : QWidget(parent), m_defaultUniverse(universe)
{
    ...
    QWidget * confWidget = new QWidget(this);
    {
        QBoxLayout * layoutConf = new QHBoxLayout(confWidget);
        {
            ...
            connect(
                ...
                this,
                &GlScopeWindow::setVerticalScaleMode);
            ...
            m_spinVertScale->setStepList(
                {50, 100, 250, E131_DATA_KEEP_ALIVE_INTERVAL_MAX, E131_NETWORK_DATA_LOSS_TIMEOUT}); // Milliseconds
            //! Milliseconds suffix
            ...
            connect(
                ...
                QOverload<int>::of(&QSpinBox::valueChanged),
                this,
                &GlScopeWindow::onVerticalScaleChanged);
            ...
            connect(
                ...
                QOverload<int>::of(&QSpinBox::valueChanged),
                this,
                &GlScopeWindow::onTimeDivisionsChanged);
...
{
    if (m_chkSyncViews->isChecked() && !running) emit stopOtherViews();

    refreshButtons();

    // Reset to start
    if (running) m_scope->setScopeView();
...
{
    if (value <= 0) value = m_spinVertScale->value();
...
void GlScopeWindow::addTrace(bool)
{
    ...
    if (m_lastTraceHue >= 360)
    {
        m_lastTraceHue -= 360;
        m_lastTraceSat -= 30;
        if (m_lastTraceSat < 0) m_lastTraceSat += 255;
    }
    ...
    uint16_t address_lo;
    {
        const QString addr_string =
            current.model()->index(current.row(), ScopeModel::COL_ADDRESS).data(Qt::DisplayRole).toString();
    ...
    if (current.column() == ScopeModel::COL_UNIVERSE)
    {
        ...
        while (m_scope->model()->addTrace(traceColor, universe, address_hi, address_lo)
               == ScopeModel::AddResult::Exists);
    }
    else
    {
        // If any other column is selected, add the next unused 8/16bit slot on the same universe
        if (universe < MIN_SACN_UNIVERSE) universe = MIN_SACN_UNIVERSE;
        do
        {
            ++address_hi;
            ...
            if (address_hi > MAX_DMX_ADDRESS)
            {
                ...
                if (address_lo != 0) address_lo = address_hi + 1;
            }
        }
        while (m_scope->model()->addTrace(traceColor, universe, address_hi, address_lo)
               == ScopeModel::AddResult::Exists);
    ...
    if (proxy)
    {
        const QModelIndex srcIndex = m_scope->model()->findFirstTraceIndex(
            universe,
            address_hi,
            address_lo,
            proxy->mapToSource(current).column());
        if (srcIndex.isValid()) m_tableView->setCurrentIndex(proxy->mapFromSource(srcIndex));
    }
}
...
void GlScopeWindow::saveTraces(bool)
{
    ...
    if (!csvExport.open(QIODevice::WriteOnly)) return;
...
void GlScopeWindow::loadTraces(bool)
{
    ...
    QFile csvExport(filename);
    if (!csvExport.open(QIODevice::ReadOnly)) return;
...
{
    if (m_chkSyncViews->isChecked()) emit startOtherViews();
}
...
void GlScopeWindow::updateTimeScrollBars()
{
    ...
    if (!m_scrollTime->isEnabled()) return;
...
void GlScopeWindow::refreshButtons()
{
    ...
    for (QWidget * w : m_disableWhenRunning) w->setEnabled(!running);
    ...
    for (QWidget * w : m_triggerSetup) w->setEnabled(!running && !m_scope->model()->triggerIsFreeRun());
...
void ColorPickerDelegate::setModelData(QWidget * editor, QAbstractItemModel * model, const QModelIndex & idx) const
{
    ...
    if (dialog)
    {
        ...
        if (color.isValid()) model->setData(idx, color, Qt::EditRole);
    }
}
