...
public:
    ...
    inline InterlockedReader(const T & item, QMutex & mutex) noexcept
        : m_value(item), m_mutex(mutex)
    {
        m_mutex.lock();
    }
...
public:
    ...
    {
        return ::std::lower_bound(begin(), end(), time, [](const QVector2D & item, float t) { return item.x() < t; });
    }
...
public:
  ...
  */
    ScopeTrace(
        ...
        float timelimit,
        size_t reservation)
        : m_color(color), m_universe(universe), m_slot_hi(address_hi - 1), m_slot_lo(address_lo - 1)
    {
    ...
    // String conversion
    static bool extractUniverseAddress(
...
public:
  ...
  */
   ...
   * @param address_lo DMX address of the Fine byte. Out of range for 8bit. (Valid range 1-512)
  */
    QModelIndex findFirstTraceIndex(uint16_t universe, uint16_t address_hi, uint16_t address_lo = 0, int column = 0)
        const;
  ...
  */
    ...
    void sACNListenerDmxReceived(
        tock packet_tock,
        int universe,
        const sACNMergedSourceList & levels,
        const sACNSource * source) final;
...
public:
  ...
  */
    bool timeInView(const QPointF & point) const
    {
        return point.x() >= m_scopeView.left() && point.x() <= m_scopeView.right();
    }
