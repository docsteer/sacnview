...
T roundCeilMultiple(T value, T multiple)
{
    ...
    return static_cast<T>(
        std::ceil(static_cast<qreal>(value) / static_cast<qreal>(multiple)) * static_cast<qreal>(multiple));
}
...
{
    if (hasRollingTimeLimit()) items = items * 2; // Reserve enough space to shuffle back as a single block
...
bool ScopeTrace::extractUniverseAddress(
    QStringView address_string,
    uint16_t & universe,
    uint16_t & address_hi,
    uint16_t & address_lo)
{
    if (address_string.front() != QLatin1Char('U')) return false;
    ...
    if (univ_str_end > 0)
    {
        ...
        if (!ok || universe == 0 || universe > MAX_SACN_UNIVERSE) return false;
...
QJsonObject ScopeTrace::toJsonConfig() const
{
    ...
    if (m_enabled) json.insert(QLatin1String("enabled"), m_enabled);
    return json;
}
...
bool ScopeTrace::setUniverse(uint16_t new_universe, bool clear_values)
{
    ...
    if (new_universe == m_universe || new_universe < 0 || new_universe > MAX_SACN_UNIVERSE) return false;

    m_universe = new_universe;

    if (clear_values) clear();
...
{
    // Validate
    if (address_hi < 1 || address_hi > MAX_DMX_ADDRESS || address_lo > MAX_DMX_ADDRESS) return false;

    if (addressHi() == address_hi && addressLo() == address_lo) return false; // No change
...
bool ScopeTrace::setAddress(QStringView addressString, bool clear_values)
{
    ...
    if (extractAddress(addressString, address_hi, address_lo)) return setAddress(address_hi, address_lo, clear_values);

    return false;
}

template<typename T, std::enable_if_t<std::is_arithmetic<T>::value, bool> = true>
inline bool fillValue(
    T & value,
    uint16_t slot_hi,
    uint16_t slot_lo,
    const sACNMergedSourceList & level_array,
    const sACNSource * source)
{
    ...
    if (slot_lo < level_array.size())
    {
        ...
        if (level_array[slot_hi].winningSource != source && level_array[slot_lo].winningSource != source) return false;

        // Do nothing if no level yet
        if (level_array[slot_hi].level < 0 || level_array[slot_lo].level < 0) return false;

        value = static_cast<uint16_t>(level_array[slot_hi].level) << 8
            | static_cast<uint16_t>(level_array[slot_lo].level);
        return true;
    }
    // 8 bit
    // Do nothing if not the winning source
    if (level_array[slot_hi].winningSource != source) return false;

    // Do nothing if no level
    if (level_array[slot_hi].level < 0) return false;

    value = static_cast<T>(level_array[slot_hi].level);
    return true;
}

void ScopeTrace::addPoint(
    float timestamp,
    const sACNMergedSourceList & level_array,
    const sACNSource * source,
    bool storeAllPoints)
...
{
    QMutexLocker lock(&m_mutex);
    for (auto & point : m_trace) point.setX(point.x() - offset);
}

ScopeModel::AddResult ScopeModel::addTrace(
    const QColor & color,
    uint16_t universe,
    uint16_t address_hi,
    uint16_t address_lo)
{
    // Verify validity
    if (!color.isValid()) return AddResult::Invalid;

    if (universe < MIN_SACN_UNIVERSE) return AddResult::Invalid;

    if (address_hi == 0) return AddResult::Invalid;

    if (universe > MAX_SACN_UNIVERSE) return AddResult::Invalid;

    if (address_hi > MAX_DMX_ADDRESS) return AddResult::Invalid;
...
ScopeModel::AddResult ScopeModel::addTrace(const QJsonObject & json)
{
    ...
    if (!ScopeTrace::extractUniverseAddress(
            json.value(QLatin1String("slot")).toString(),
            universe,
            address_hi,
            address_lo))
        return AddResult::Invalid;

    const AddResult result = addTrace(
        QColor(json.value(QLatin1String("color")).toString()),
        universe,
        address_hi,
        address_lo);
...
{
    auto univ_it = m_traceLookup.find(universe);
    if (univ_it == m_traceLookup.end()) return;
...
void ScopeModel::removeTraces(const QModelIndexList & indexes)
{
    ...
    {
        if (prevRow == row) continue; // This was a duplicate
...
ScopeTrace * ScopeModel::findTrace(uint16_t universe, uint16_t address_hi, uint16_t address_lo)
{
    ...
    if (univ_it != m_traceLookup.end())
    {
        ...
        {
            if (item->addressHi() == address_hi && item->addressLo() == address_lo) return item;
        }
    }
...
QModelIndex ScopeModel::findFirstTraceIndex(uint16_t universe, uint16_t address_hi, uint16_t address_lo, int column)
    const
{
    for (int row = 0; row < m_traceTable.size(); ++row)
    {
        ...
        if (trace
            ...
            && trace->addressHi() == address_hi
            && trace->addressLo() == address_lo)
        {
...
{
    if (m_traceTable.empty()) return Qt::Checked; // Any that are added will be checked
    ...
    for (const ScopeTrace * trace : m_traceTable)
    {
        ...
        switch (result)
        {
            case Qt::PartiallyChecked: result = trace->enabled() ? Qt::Checked : Qt::Unchecked; break;
            case Qt::Unchecked:
                if (trace->enabled()) return Qt::PartiallyChecked;
                break;
            case Qt::Checked:
                if (!trace->enabled()) return Qt::PartiallyChecked;
                break;
...
{
    if (check == Qt::PartiallyChecked) return;
    ...
    if (index.column() < COL_COUNT && index.row() < rowCount())
    {
        ...
        if (trace)
        {
            switch (index.column())
            {
                default: break;
                case COL_UNIVERSE:
                    if (role == Qt::DisplayRole || role == Qt::EditRole || role == DataSortRole)
...
bool ScopeModel::setData(const QModelIndex & idx, const QVariant & value, int role)
{
    ...
    if (idx.column() >= COL_COUNT || idx.row() >= rowCount()) return false;
    ...
    switch (idx.column())
    {
        ...
        case COL_UNIVERSE:
            ...
            if (role == Qt::EditRole)
            {
                ...
                {
                    if (isTrigger) m_trigger.setTrigger(*trace);
        ...
        case COL_TRIGGER:
            if (role == Qt::CheckStateRole)
            {
                if (trace->isValid())
                {
                    ...
                    emit dataChanged(
                        index(0, COL_TRIGGER),
                        index(rowCount() - 1, COL_TRIGGER),
                        {Qt::CheckStateRole, DataSortRole});
                    return true;
                }
            }
            break;
...
void ScopeModel::private_removeAllTraces()
{
    ...
    for (ScopeTrace * trace : m_traceTable) delete trace;
...
QString ScopeModel::captureConfigurationString() const
{
    ...
    if (m_runTime > 0) result.append(QStringLiteral("Run For %1 sec,").arg(m_runTime));
...
qreal ExtractValue(const QString & configString, const QString & item, const QString & units, qreal def = 0)
{
    ...
    if (valuePos < 0 || valueUnitPos < 0) return def;
...
bool ScopeModel::saveTraces(QIODevice & file) const
{
    ...
    out
        << CaptureOptionsTitle
        << QLatin1String(":,")
        << captureConfigurationString()
        << QStringLiteral(",hh:mm:ss.000");
    out << "\n\n";
    ...
    struct ValueItem
    {
        ValueItem(const ScopeTrace * trace)
            : values(trace->values().value())
        {
    ...
    while (in.readLineInto(&line))
    {
        ...
        else if (line.contains(RowTitleColor))
        {
            ...
            if (result.universes.startsWith(ColumnTitleTimestamp)
                || result.universes.startsWith(ColumnTitleWallclockTime))
            {
                return result;
            }
...
bool ScopeModel::loadTraces(QIODevice & file)
{
    ...
    if (title_line.universes.isEmpty()) return false;
    ...
    for (int i = 0; timeColumn == -1 && i < colors.size(); ++i)
    {
        ...
        // Labels are optional
        if (!labels.isEmpty()) labels.pop_front();
    }

    // Time is required
    if (timeColumn < 0) return false;

    const int firstTraceColumn = timeColumn + 1; // Column index of first trace

    // Remove empty colors from the end
    while (colors.last().isEmpty()) colors.pop_back();
    // Remove empty items from the end
    while (titles.last().isEmpty()) titles.pop_back();

    if (colors.size() != titles.size() || titles.empty()) return false; // No or invalid data
    ...
    if (wallclockTrigger.isValid())
        ...
        m_wallclockTrigger_ms = QDateTime::fromString(QStringLiteral("1975-01-01 00:00:00.000"), DateTimeFormatString)
                                    .toMSecsSinceEpoch();
    ...
    for (qsizetype i = 0; i < titles.size(); ++i)
    {
        ...
        if (!ScopeTrace::extractUniverseAddress(
                full_title.trimmed(),
                univ_slots.universe,
                univ_slots.address_hi,
                univ_slots.address_lo))
        {
        ...
        if (addTrace(label, color, univ_slots.universe, univ_slots.address_hi, univ_slots.address_lo)
            == AddResult::Added)
        {
    ...
    while (in.readLineInto(&data_line))
    {
        ...
        for (size_t i = 0; i < traces.size(); ++i)
        {
            ...
            if (trace)
            {
                ...
                if (ok) trace->addValue({timestamp, level});
            }
        }
    }
...
{
    if (runTime() == seconds) return;

    // Validate and emit new or existing value
    if (seconds >= 0) m_runTime = seconds;
...
{
    if (storageTime() == seconds) return;

    // Validate and update new or existing value
    if (seconds >= 0) m_storageTime = seconds;
...
bool ScopeModel::moveTrace(ScopeTrace * trace, uint16_t new_universe, bool clear_values)
{
    ...
    if (!trace->setUniverse(new_universe, clear_values)) return false; // New universe invalid or unchanged
    ...
    if (univ_it == m_traceLookup.end())
    {
        ...
        // Maybe start listening
        if (isRunning()) addListener(new_universe);
    }
    else
    {
        // Add this new trace to the existing universe
...
{
    if (m_storageTime > 0) return QRectF(std::max(m_endTime - m_storageTime, 0.0), 0, m_storageTime, m_maxValue);
    return QRectF(0, 0, m_endTime, m_maxValue);
}

qreal ScopeModel::endTime() const
{
    if (isRunning() && isTriggered()) return (qreal(sACNManager::elapsed()) / 1000) - m_startOffset;
...
{
    if (m_wallclockTrigger_ms == 0) return false;

    datetime.setMSecsSinceEpoch(m_wallclockTrigger_ms + ((time + m_startOffset) * 1000));
    return true;
}

void ScopeModel::sACNListenerDmxReceived(
    tock packet_tock,
    int universe,
    const sACNMergedSourceList & levels,
    const sACNSource * source)
{
    if (!m_running) return;
    ...
    if (it == m_traceLookup.end()) return;
    ...
    if (!isTriggered())
    {
        ...
        if (universe == m_trigger.universe
            && fillValue(current_level, m_trigger.address_hi - 1, m_trigger.address_lo - 1, levels, source))
        {
            ...
            switch (m_trigger.mode)
            {
                default: break;
                case Trigger::Above:
                    if (current_level > m_trigger.level) triggerNow(timestamp);
                    break;
                case Trigger::Below:
                    if (current_level < m_trigger.level) triggerNow(timestamp);
                    break;
                case Trigger::LevelCross:
                    if ((m_trigger.last_level == m_trigger.level && current_level != m_trigger.level) // Was at, now not
                        || (m_trigger.last_level > m_trigger.level
                            && current_level < m_trigger.level) // Was above, now below
                        || (m_trigger.last_level != -1
                            && m_trigger.last_level < m_trigger.level
                            && current_level > m_trigger.level) // Was below, now above
                    )
...
{
    if (timeDivisions() == milliseconds) return;
...
{
    if (width == m_levelDotSize) return;
...
void GlScopeWidget::ShaderProgram::BuildProgram(
...
inline static void DrawLevelAxisText(
    QPainter & painter,
    const QFontMetricsF & metrics,
    const QRectF & scopeWindow,
    int level,
    qreal y_scale,
...
void GlScopeWidget::paintGL()
{
    ...
    if (scopeWindow.width() < AXIS_TICK_SIZE) return;
    ...
    QFontMetricsF metrics(font);
    {
        ...
        // Draw vertical (level) axis
        {
            ...
            switch (m_verticalScaleMode)
            {
                ...
                case VerticalScale::DeltaTime: postfix = QStringLiteral("ms"); [[fallthrough]];
                case VerticalScale::Dmx8:
                case VerticalScale::Dmx16:
                {
                ...
                case VerticalScale::Invalid: break;
            }
        }
        ...
        if (m_timeFormat == TimeFormat::Elapsed)
        {
            ...
            for (qreal time = roundCeilMultiple(m_scopeView.left(), m_timeInterval); time < m_scopeView.right() + 0.001;
                 time += m_timeInterval)
            {
                // Grid lines in trace space
                ...
                const QString text = milliseconds ? QStringLiteral("%1ms").arg(time * 1000.0)
                                                  : QStringLiteral("%1s").arg(time);
                QRectF fontRect = metrics.boundingRect(text);
        ...
        else
        {
                 ...
                 time += m_timeInterval)
            {
                ...
                const QString text = x_scale > 80.0 ? datetime.toString(TimeFormatString)
                                                    : datetime.toString(ShortTimeFormatString);
    ...
    if (!m_cursorPoint.isNull())
    {
        ...
        if (levelInView(m_cursorPoint))
        {
            ...
            if (m_verticalScaleMode != VerticalScale::DeltaTime)
            {
                ...
                const qreal percent_value = m_cursorPoint.y()
                    * (m_verticalScaleMode == VerticalScale::Dmx16 ? 100.0f / ScopeModel::kMaxDmx16
                                                                   : 100.0f / ScopeModel::kMaxDmx8);
        ...
        if (timeInView(m_cursorPoint))
        {
            ...
            if (x + textWidth > scopeWindow.width()) x -= (textWidth + 2.0 * AXIS_TO_WINDOW_GAP);

            painter.drawText(
                QPointF(x, timeAtTop ? metrics.height() : scopeWindow.height() - AXIS_TO_WINDOW_GAP),
                text);
        }
    }
}
...
{
    if (ev->buttons() & Qt::LeftButton) updateCursor(ev->pos());
...
void GlScopeWidget::updateCursor(const QPoint & widgetPos)
{
    ...
    if (pos.x() < AXIS_LABEL_WIDTH && pos.x() > AXIS_LABEL_WIDTH - AXIS_TICK_SIZE) pos.setX(AXIS_LABEL_WIDTH);
...
bool ScopeModel::TriggerConfig::isTrigger() const
{
    ...
    if (mode == Trigger::FreeRun) return false;

    if (universe > 0
        && universe <= MAX_SACN_UNIVERSE
        && address_hi > 0
        && address_hi <= MAX_DMX_ADDRESS
        && address_lo <= MAX_DMX_ADDRESS)
    {
        ...
        if (mode == Trigger::Above && level == max_level) return false;

        // Can't fall below 0
        if (mode == Trigger::Below && level == 0) return false;
...
void ScopeModel::TriggerConfig::setConfiguration(const QString & configString)
{
    ...
    else
    {
        ...
        if (ScopeTrace::extractUniverseAddress(
                configString.mid(triggerUnivPos + 1, triggerLevelPos - triggerUnivPos - 1),
                new_univ,
                new_addr_hi,
                new_addr_lo))
        {
        ...
        bool ok;
        const qsizetype triggerLevel = configString.mid(triggerLevelPos + 1, triggerLevelEnd - triggerLevelPos)
                                           .toInt(&ok);
        if (ok) level = triggerLevel;
    }
}
