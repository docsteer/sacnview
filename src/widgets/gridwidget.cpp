...
void GridWidget::paintEvent(QPaintEvent * event)
{
    ...
    for (int row = 1; row < ROW_COUNT + 1; row++)
    {
        ...
        QString rowLabel = QString("%1-%2").arg(1 + (row - 1) * 32).arg((row) * 32);
    ...
    {
        QRect textRect(
            (FIRST_COL_WIDTH + col * CELL_WIDTH) * scaleWidth,
            0,
            CELL_WIDTH * scaleWidth,
            m_cellHeight * scaleHeight);
        QString rowLabel = QString("%1").arg(col + 1);
    ...
    for (int row = 0; row < ROW_COUNT; row++)
        for (int col = 0; col < COL_COUNT; col++)
        {
            ...
            QRect textRect(
                (FIRST_COL_WIDTH + col * CELL_WIDTH) * scaleWidth,
                (row + 1) * m_cellHeight * scaleHeight,
                CELL_WIDTH * scaleWidth,
                m_cellHeight * scaleHeight);
...
int GridWidget::cellHitTest(const QPoint & point)
{
    ...
    if (!drawnRect.contains(point)) return -1;
    ...
    if (m_allowMultiSelect)
    {
        ...
        if ((event->modifiers() & Qt::ShiftModifier) && m_selectedAddresses.count() > 0)
        {
            ...
            for (int i = low; i <= high; i++)
                if (!m_selectedAddresses.contains(i)) m_selectedAddresses << i;
            update();
            emit selectedCellsChanged(m_selectedAddresses);
            return;
        }
...
void GridWidget::keyPressEvent(QKeyEvent * event)
{
    ...
    {
        m_selectedAddresses.clear();
        for (int i = 0; i < 512; i++) m_selectedAddresses << i;
        emit selectedCellsChanged(m_selectedAddresses);
        update();
